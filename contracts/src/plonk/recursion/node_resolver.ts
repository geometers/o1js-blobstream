...
const resolveLeafProof = async (index: number, side: LeftOrRight): Promise<ZkpProofLeft | ZkpProofRight> => {
    ...
    if (index >= NUM_OF_ZKPS) {
        const pLast = await ZkpProofLeft.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/proofs/layer0/zkp${NUM_OF_ZKPS - 1}.json`, 'utf8')));
    ...
    if (side === LeftOrRight.LEFT) {
        ...
        return await ZkpProofLeft.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/proofs/layer0/zkp${index}.json`, 'utf8'))); 
    } else {
        // console.log("reading zkp: ", index);
        return await ZkpProofRight.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/proofs/layer0/zkp${index}.json`, 'utf8')));
    }
}
...
const resolveLeafVk = async (index: number): Promise<VerificationKey> => {
    ...
    if (index >= NUM_OF_ZKPS) {
        return await VerificationKey.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/vks/vk${0}.json`, 'utf8')));
    }

    // console.log("reading vk: ", index);
    return await VerificationKey.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/vks/vk${index}.json`, 'utf8')));
}
...
const proveLayer1 = async (index: number) => {
    ...
    fs.writeFileSync(`${workDir}/plonk/recursion/proofs/layer1/p${index}.json`, JSON.stringify(proof), 'utf8');
    console.log(`layer: 1 node: ${index} written`);
}

const proveLayer2 = async (index: number) => {
    const layer1Vk = await VerificationKey.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/vks/layer1Vk.json`, 'utf8')));
    await node.compile({cache: Cache.FileSystem(cacheDir)});

    const leftIdx = index * 2; 
    const rightIdx = leftIdx + 1; 

    const piLeft = await NodeProofLeft.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/proofs/layer1/p${leftIdx}.json`, 'utf8')));
    const piRight = await NodeProofRight.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/proofs/layer1/p${rightIdx}.json`, 'utf8')));

    const proof = await node.compute(piLeft, layer1Vk, piRight, layer1Vk, Field(2));
    fs.writeFileSync(`${workDir}/plonk/recursion/proofs/layer2/p${index}.json`, JSON.stringify(proof), 'utf8');
...
const prove = async (layer: number, index: number) => {
    if (layer === 1) {
        ...
        const nodeVk = await VerificationKey.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/vks/nodeVk.json`, 'utf8')));
        
        const piLeft = await NodeProofLeft.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/proofs/layer${layer - 1}/p${leftIdx}.json`, 'utf8')));
        const piRight = await NodeProofRight.fromJSON(JSON.parse(fs.readFileSync(`${workDir}/plonk/recursion/proofs/layer${layer - 1}/p${rightIdx}.json`, 'utf8')));

        const proof = await node.compute(piLeft, nodeVk, piRight, nodeVk, Field(layer));
        fs.writeFileSync(`${workDir}/plonk/recursion/proofs/layer${layer}/p${index}.json`, JSON.stringify(proof), 'utf8');
