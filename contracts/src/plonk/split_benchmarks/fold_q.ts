...
import { compute_alpha_square_lagrange_0, evalVanishing, fold_quotient } from '../piop/plonk_utils.js';
...
function main() {
    ...
    let zeta = Provable.witness(FrC.provable, () => FrC.from("0x0097228875a04c12dda0a76b705856f1a99fd19613c0ba69b056f4c4d18921e5"))
    const [zeta_pow_n, zh_eval] = evalVanishing(zeta, VK)
    // const alpha_2_l0 = compute_alpha_square_lagrange_0(zh_eval, zeta, zeta, VK); 
    const [hx, hy] = fold_quotient(proof.h0_x, proof.h0_y, proof.h1_x, proof.h1_y, proof.h2_x, proof.h2_y, zeta, zeta_pow_n, zh_eval)

}


// npm run build && node --max-old-space-size=65536 build/src/plonk/split_benchmarks/fold_q.js
(async () => {
